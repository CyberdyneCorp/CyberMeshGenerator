...
detailed behavior lives in the referenced capability specs (e.g. [language-bindings](../language-bindings/spec.md)).
...
- **WHEN** the library is installed to a prefix and a separate project requests it through the
  standard discovery mechanism (e.g. `find_package(CyberMeshGenerator)`) and links its exported
  target(s) (e.g. `cmg::cmg`)
...
any third-party dependencies resolved automatically (from a package manager or a pinned fetch)
rather than a manual multi-repository setup. Absence of an optional component SHALL degrade
gracefully, never breaking the default build.
...
- **THEN** the portable CPU-only baseline SHALL build with only the C++ standard library, and any
  optional backend or integration left off SHALL not affect that build
...
- **THEN** the native library SHALL be built and bundled automatically and importing/using the
  binding SHALL work, with an environment override (e.g. `CMG_C_LIB`) available to point at a
...
the supported profiles (CPU baseline, sanitizers, mobile single-precision, bindings), and any
project-artifact validation (specs, linters) SHALL run. A pull request SHALL NOT be mergeable
while its build, tests, or validation are failing.
...
Any performance claim the project makes SHALL be backed by a runnable benchmark or comparison and
by recorded reference results (hardware, configuration, and measured metric) kept in the docs, so
claims are reproducible and comparable rather than anecdotal.

#### Scenario: A performance claim is reproducible
- **WHEN** the docs state a throughput/accuracy figure (e.g. "~99% identical tetrahedra vs TetGen")
- **THEN** a documented command or example SHALL reproduce that measurement, and the recorded
  reference results SHALL name the hardware, dataset, and configuration they were taken on
...
- **THEN** the version SHALL follow semver, the changelog SHALL record what changed, and any
  shipped shared library (e.g. the C ABI `libcmg_c`) SHALL carry an ABI version consumers can
  pin against
...
- **THEN** they SHALL be able to build or install the library and run a documented example
  (e.g. one under `examples/`) without undocumented steps
